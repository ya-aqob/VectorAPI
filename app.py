...
INSERT_USER_LEADERBOARD = ("INSERT INTO leaderboard (id, points) VALUES (%L, %L)") # creates new user in leaderboard table
UPDATE_NEW_POINTS = "UPDATE leaderboard SET points = (%L) WHERE id = (%L); " # adds points for given user
FIND_USER_ID = "SELECT userid from userinfo WHERE username = (%s)" # grabs userid based on what username is
SELECT_PASSWORD_VERIFICATION = "SELECT password, salt from userinfo WHERE userid = (%L)" # grabs saved hashed password and salt
